...
public class HeroesBackendApplicationTests {
	...
	/**
	 * Delete a superhero and ensure it is well and truly deleted.
	 * 
	 * @throws Exception
	 */
	@Test
	public void shouldDeleteSuperhero() throws Exception {
		...
		invokeGetHero(heroId)
			.andExpect(status().isOk());
		
		//Delete it.
		invokeDeleteHero(heroId)
			.andExpect(status().isNoContent());
		
		//List, previously of length 1, now empty
		assertThat(getAllHeroes().length)
			.isEqualTo(0);
	...
	/**
	 * Deleting a hero for an id that does not exist in database should return HTTP 404 - not found.
	 * 
	 * @throws Exception
	 */
	@Test
	public void should404DeleteNotfoundSuperhero() throws Exception {
		...
		invokeDeleteHero(heroes[0].getId()+1)
			.andExpect(status().isNotFound());
	}
	 ...
	 */
	@Test
	public void shouldSearchNotFoundSuperherosEmptyList() throws Exception {
	...
	private Hero[] getAllHeroes() throws Exception {
		Hero[] heroes =  fromJsonResult(
				invokeAllHeroes().andReturn(),
				Hero[].class);
		return heroes;
	}
	...
	private ResultActions invokeSearchHeroes(String term) throws Exception {
		return mvc.perform(get(BASE_URL + "search/name?contains=" + term)
				.accept(MediaType.APPLICATION_JSON));
	}
